...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    exit;
        ...
        echo "<p>Chyba pri aktualizácii produktu.</p>";
    }
}

$product = $productObj->getById($id);
if (!$product) {
    die("Produkt sa nenašiel.");
}
...
<form method="post">
    ...
    <label>Druh dreva:<br>
        <select name="wood_type_id" required>
            <?php foreach ($woodTypes as $type): ?>
                <option value="<?= $type['id'] ?>" <?= $type['id'] == $product['wood_type_id'] ? 'selected' : '' ?>>
    ...
    <label>Rok výroby:<br>
        <input type="number" name="rok" value="<?= htmlspecialchars($product['rok']) ?>" required>
    </label><br><br>
